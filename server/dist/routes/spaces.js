...
router.get('/', auth_js_1.optionalAuth, async (req, res) => {
    ...
    `;
        ...
        const enriched = rows.map(s => {
            ...
            let memberRole = null;
            if (req.user) {
                const mem = (0, db_js_1.queryOne)('SELECT role FROM memberships WHERE spaceId = ? AND userId = ?', [s.id, req.user.id]);
                if (mem) {
                    isMember = true;
                    memberRole = mem.role;
                }
...
router.get('/:id', auth_js_1.optionalAuth, async (req, res) => {
    try {
        const { id } = req.params;
        let space = (0, db_js_1.queryOne)(`SELECT s.*,
              u.id as owner_id, u.name as owner_name, u.username as owner_username, u.avatarUrl as owner_avatarUrl, u.bio as owner_bio,
              (SELECT COUNT(*) FROM memberships WHERE spaceId = s.id) as membersCount,
              (SELECT COUNT(*) FROM channels WHERE spaceId = s.id) as channelsCount
       FROM spaces s
       JOIN users u ON s.ownerId = u.id
       WHERE s.id = ? OR s.slug = ?`, [id, id]);
        if (!space) {
            const courseRow = (0, db_js_1.queryOne)(`SELECT c.*, u.id as owner_id, u.name as owner_name, u.username as owner_username, u.avatarUrl as owner_avatarUrl, u.bio as owner_bio
         FROM courses c
         JOIN users u ON c.creatorId = u.id
         WHERE c.id = ? OR c.slug = ?`, [id, id]);
            if (courseRow) {
                if (courseRow.spaceId) {
                    space = (0, db_js_1.queryOne)(`SELECT s.*,
                    u.id as owner_id, u.name as owner_name, u.username as owner_username, u.avatarUrl as owner_avatarUrl, u.bio as owner_bio,
                    (SELECT COUNT(*) FROM memberships WHERE spaceId = s.id) as membersCount,
                    (SELECT COUNT(*) FROM channels WHERE spaceId = s.id) as channelsCount
             FROM spaces s
             JOIN users u ON s.ownerId = u.id
             WHERE s.id = ?`, [courseRow.spaceId]);
                }
                if (!space) {
                    space = {
                        id: courseRow.id,
        ...
        let channels = (0, db_js_1.queryAll)('SELECT * FROM channels WHERE spaceId = ? ORDER BY orderIndex ASC', [space.id]);
        ...
        const courses = (0, db_js_1.queryAll)('SELECT id, title, slug, thumbnailUrl, difficulty, durationHours, price, isFree FROM courses WHERE spaceId = ?', [space.id]);
        const members = (0, db_js_1.queryAll)(`SELECT m.*, u.id as user_id, u.name, u.username, u.avatarUrl, u.role as user_role, u.xp, u.level
       FROM memberships m
       ...
       LIMIT 30`, [space.id]);
        ...
        let memberRole = null;
        if (req.user) {
            const mem = (0, db_js_1.queryOne)('SELECT role FROM memberships WHERE spaceId = ? AND userId = ?', [space.id, req.user.id]);
            if (mem) {
                isMember = true;
                memberRole = mem.role;
            }
...
router.post('/:id/join', auth_js_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const now = new Date().toISOString();
        const space = (0, db_js_1.queryOne)('SELECT id, name FROM spaces WHERE id = ? OR slug = ?', [id, id]);
        if (!space) {
            return res.status(404).json({ error: 'Comunidade não encontrada' });
        }
        const existing = (0, db_js_1.queryOne)('SELECT id FROM memberships WHERE spaceId = ? AND userId = ?', [space.id, userId]);
        if (!existing) {
            (0, db_js_1.execute)(`INSERT INTO memberships (id, userId, spaceId, role, joinedAt) VALUES (?, ?, ?, 'MEMBER', ?)`, [`mem-${Date.now()}`, userId, space.id, now]);
        }
        // Also auto-enroll in all courses linked to this space
        const linkedCourses = (0, db_js_1.queryAll)('SELECT id FROM courses WHERE spaceId = ? OR id = ?', [space.id, space.id]);
        linkedCourses.forEach(c => {
            (0, db_js_1.execute)(`INSERT OR IGNORE INTO enrollments (id, userId, courseId, enrolledAt, progressPercent) VALUES (?, ?, ?, ?, 0)`, [`enr-${Date.now()}-${c.id}`, userId, c.id, now]);
        });
