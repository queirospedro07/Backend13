...
router.get('/stats', auth_js_1.authenticate, (0, auth_js_1.requireRole)('CREATOR', 'ADMIN'), async (req, res) => {
    try {
        const creatorId = req.user.id;
        const courses = (0, db_js_1.queryAll)(`SELECT c.*,
       ...
       WHERE c.creatorId = ?`, [creatorId]);
        ...
        const recentEnrollments = (0, db_js_1.queryAll)(`SELECT e.*, u.id as user_id, u.name, u.username, u.avatarUrl, u.email, c.title as course_title
       FROM enrollments e
...
router.get('/courses', auth_js_1.authenticate, (0, auth_js_1.requireRole)('CREATOR', 'ADMIN'), async (req, res) => {
    try {
        const creatorId = req.user.id;
        const courses = (0, db_js_1.queryAll)(`SELECT c.*,
              (SELECT COUNT(*) FROM enrollments WHERE courseId = c.id) as studentsCount,
...
router.post('/courses', auth_js_1.authenticate, (0, auth_js_1.requireRole)('CREATOR', 'ADMIN'), async (req, res) => {
    try {
        const creatorId = req.user.id;
        const { title, description, category, difficulty, language, price, isFree, thumbnailUrl, bannerUrl, durationHours, spaceId: customSpaceId, channels, modules, } = req.body;
        if (!title || !category) {
        ...
        (0, db_js_1.transaction)(() => {
            ...
            if (!resolvedSpaceId) {
                ...
                (0, db_js_1.execute)(`INSERT INTO spaces (id, name, slug, description, iconUrl, bannerUrl, isPrivate, category, ownerId, memberCount, createdAt)
           VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, 1, ?)`, [
                    resolvedSpaceId,
                    `Comunidade ${title}`,
                    spaceSlug,
                ...
                (0, db_js_1.execute)(`INSERT INTO memberships (id, userId, spaceId, role, joinedAt) VALUES (?, ?, ?, 'OWNER', ?)`, [`mem-${Date.now()}`, creatorId, resolvedSpaceId, now]);
                // Insert channels
                ...
                channelsToInsert.forEach((ch, chIdx) => {
                    ...
                    (0, db_js_1.execute)(`INSERT INTO channels (id, spaceId, name, type, topic, orderIndex, isVoice, isLocked, guidingQuestion, guidelines)
             VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`, [
                        chId,
                        resolvedSpaceId,
            ...
            // 2. Insert Course
            (0, db_js_1.execute)(`INSERT INTO courses (id, title, slug, description, category, difficulty, thumbnailUrl, bannerUrl, price, isFree, isPublished, featured, durationHours, language, creatorId, spaceId, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0, ?, ?, ?, ?, ?, ?)`, [
                courseId,
                title,
                slug,
            ...
            if (Array.isArray(modules)) {
                modules.forEach((mod, mIdx) => {
                    ...
                    (0, db_js_1.execute)(`INSERT INTO course_modules (id, courseId, title, description, orderIndex) VALUES (?, ?, ?, ?, ?)`, [modId, courseId, mod.title || `Módulo ${mIdx + 1}`, mod.description || '', mIdx]);
                        ...
                        mod.lessons.forEach((les, lIdx) => {
                            ...
                            const contentToStore = typeof les.content === 'object'
                                ...
                                : les.customData
                                    ...
                                    : les.content || '';
                            (0, db_js_1.execute)(`INSERT INTO lessons (id, moduleId, title, type, content, videoUrl, durationMin, orderIndex, xpReward)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                                lesId,
                                modId,
                            ...
                            if (les.quiz) {
                                ...
                                (0, db_js_1.execute)(`INSERT INTO quizzes (id, lessonId, title, description, passingScore, xpReward)
                   VALUES (?, ?, ?, ?, ?, ?)`, [
                                    quizId,
                                    lesId,
                                ...
                                if (Array.isArray(les.quiz.questions)) {
                                    les.quiz.questions.forEach((q, qIdx) => {
                                        ...
                                        (0, db_js_1.execute)(`INSERT INTO quiz_questions (id, quizId, question, options, correctOptionIndex, explanation, orderIndex)
                       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                                            qId,
                                            quizId,
            ...
            (0, db_js_1.execute)(`INSERT OR IGNORE INTO enrollments (id, userId, courseId, enrolledAt, progressPercent) VALUES (?, ?, ?, ?, 100)`, [`enr-${Date.now()}`, creatorId, courseId, now]);
        });
        const createdCourse = (0, db_js_1.queryOne)('SELECT * FROM courses WHERE id = ?', [courseId]);
